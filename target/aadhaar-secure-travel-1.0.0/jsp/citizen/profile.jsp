...
<%@ page import="com.aadhaar.model.*, com.aadhaar.util.Database" %>
<%
        Citizen citizen = (Citizen) request.getAttribute("citizen");
    String success  = (String) request.getAttribute("success");
    if (citizen == null) {
        response.sendRedirect(cp + "/citizen/list");
        return;
    }
    Database db = Database.getInstance();
    Passport passport = db.getPassportByUid(citizen.getUid());
    DrivingLicense license = db.getLicenseByUid(citizen.getUid());
%>
...
<div class="card">
  ...
  <div class="profile-grid">
    <div>
      ...
      <div class="uid-display" style="font-size:1rem;padding:6px 10px;margin-top:4px"><%= citizen.getUid() %></div>
    </div>
    <div>
      <div class="info-grid">
        <div class="info-item"><div class="lbl">Full Name</div><div class="val" style="font-size:1.2rem;font-family:'Rajdhani',sans-serif;font-weight:700"><%= citizen.getName() %></div></div>
...
<div class="card">
  ...
  <% if (passport != null) { %>
  <div class="info-grid">
    <div class="info-item"><div class="lbl">Passport Number</div><div class="val mono"><%= passport.getPassportNumber() %></div></div>
    ...
    <div class="info-item"><div class="lbl">Issuing Authority</div><div class="val"><%= passport.getIssuingAuthority() %></div></div>
    <div class="info-item"><div class="lbl">Status</div>
      <div class="val"><span class="badge badge-<%= passport.getStatus() == Passport.PassportStatus.ACTIVE ? "active" : "danger" %>"><%= passport.getStatus() %></span></div>
    </div>
  </div>
  <% } else { %>
  <div style="color:var(--muted);padding:1rem 0">No passport linked.
    <a href="<%= cp %>/passport/issue?uid=<%= citizen.getUid() %>" class="btn btn-primary btn-sm" style="margin-left:12px">📘 Issue Passport</a>
  </div>
  <% } %>
</div>
...
<div class="card">
  ...
  <div class="info-grid">
    ...
    <div class="info-item"><div class="lbl">Status</div>
      <div class="val">
        <%
          ...
          String lbadge = "badge-info";
          if ("ISSUED".equals(ls)) lbadge = "badge-active";
          else if ("DENIED".equals(ls)) lbadge = "badge-danger";
          else if ("PENDING".equals(ls)) lbadge = "badge-pending";
        %>
    ...
    <div class="info-item"><div class="lbl">Test Result</div><div class="val"><%= license.getTestResult() != null ? license.getTestResult() : "Pending" %></div></div>
    <% } %>
  </div>
  <% } else { %>
  <div style="color:var(--muted);padding:1rem 0">No driving licence on record.
    <a href="<%= cp %>/license/apply?uid=<%= citizen.getUid() %>" class="btn btn-primary btn-sm" style="margin-left:12px">🚗 Apply</a>
  </div>
  <% } %>
</div>
...
<div class="card">
  ...
  <form method="post" action="<%= cp %>/citizen/verify" style="display:flex;gap:1rem;align-items:flex-end">
  ...
  <% Boolean authResult = (Boolean) request.getAttribute("authResult");
    ...
    <br/>
    <% if (authResult) { %>
    <div class="alert alert-success">✅ &nbsp;PIN verified successfully. Identity authenticated.</div>
    <% } else { %>
    <div class="alert alert-error">❌ &nbsp;Incorrect PIN. Access denied.</div>
    <% } %>
  <% } %>
</div>
